class Sucursal {
    ...
    static crear_sucursal(nombre) {
        if (Sucursal.sucursales.includes(nombre)) {
            console.log("Esta sucursal ya ha sido registrada con anterioridad.");
            return null;
        } else {
            const suc = new Sucursal(nombre);
            Sucursal.sucursales.push(suc);
            return suc;
        }
    }

    agregar_celular(celular){
        this.celulares.push(celular)
        console.log(`El teléfono ${celular.marca} ${celular.modelo} ha sido añadido a la sucursal ${this.nombre}`)
    }
...
class Celular {
    constructor(serie, imei, marca, modelo, sucursal) {
        this.serie = serie;
        this.imei = imei;
        this.marca = marca;
        this.modelo = modelo;
    ...
    static crear_celular(serie, imei, marca, modelo) {
        if (Celular.reg_series.includes(serie) || Celular.reg_imei.includes(imei)) {
            console.log("Este teléfono ya ha sido registrado.");
            return null;
        } else {
            const tel = new Celular(serie, imei, marca, modelo);
            Celular.reg_series.push(serie);
            Celular.reg_imei.push(imei);
            return tel;
        }
    }
...
"${mensaje}"`)
    ...
    empezar_reparacion(){
        if (this.pago_parcial && this.autorizacion){
            if(this.diagnosticos.length != 0){
                ...
                console.log(`El celular está siendo reparado y ${this.tecnico_encargado.nombre} es el técnico encargado de repararlo.`)
            } else {
                console.log("El celular aún no ha sido diagnosticado.")
            } 
        } else {console.log("No se ha dado la autorización del usuario o el pago del 50%")}
    }
    ...
    registrar_reparo(){
        if(this.diagnosticos.length != 0){
            ...
            console.log(`El celular ha sido reparado con éxito y ${this.tecnico_encargado.nombre} fue el técnico encargado.`)
        } else {
            console.log("El celular aún no ha sido diagnosticado.")
        }
    }
...
class Tecnicos {
    constructor(nombre,experiencia,sucursal,...especializaciones){
        this.nombre = nombre;
